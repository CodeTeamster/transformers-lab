from transformers import ViTRDForImageClassification
...
class EViTTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        if return_outputs:
            os.environ["EViT"] = str(1 - self._discard_rate)
            output = model(**inputs)
            os.environ.pop("EViT", None)
...
def run_train(args: argparse.Namespace):
    ...
    if int(os.environ.get("LOCAL_RANK", 0)) == 0:
        print(f"[INITIAL_MODEL]: Instantiate a model from a pretrained model: {args.pretrained_model_path}")
