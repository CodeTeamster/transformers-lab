...
def run_eval(args: argparse.Namespace):
    ...
    if args.discard_rate > 0.0 and len(args.discard_before_layers) > 0:
        discard_before_layers = [False] * model.config.vision_config.num_hidden_layers
    ...
    flops, params = calculate_flops(
        ...
        sys_token_len=11,
        seq_len=600,
        vision_token_len=(model.config.vision_config.image_size // model.config.vision_config.patch_size) ** 2,
    )
